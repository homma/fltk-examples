...
struct Point {
  ...
  static auto from_angle_distance(float degree, float distance) -> Point;
  auto translate(Point &move) -> void;
};

auto Point::from_angle_distance(float degree, float distance) -> Point {
...
auto Point::translate(Point &move) -> void {
  this->x += move.x;
  this->y += move.y;
}
